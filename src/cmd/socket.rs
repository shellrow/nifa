...
use netsock::{family::AddressFamilyFlags, protocol::ProtocolFlags, socket::SocketInfo};
...
pub fn show_sockets(_cli: &Cli, args: &SocketArgs) -> Result<()> {
    ...
    if let Some(ref state) = args.state {
        ...
        socks.retain(|s| match &s.protocol_socket_info {
    ...
    match args.export {
        Some(export_format) => {
            export(export_format, args.output.as_deref(), &socks)?;
            return Ok(());
        }
...
pub fn print_socket_table(socks: &[SocketInfo]) {
    ...
    let mut table = crate::renderer::table::make_table(&[
        "Proto",
        "Fam",
        "Local Address",
        "Remote Address",
